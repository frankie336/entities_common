...
class VectorStoreCreate(BaseModel):
    shared_id: str = Field(..., description="Client-generated unique ID (e.g., vs_...). Used for DB primary key "
                                            "and " "Qdrant collection name.")
...
class VectorStoreRead(BaseModel):
    id: str = Field(..., description="Unique identifier for the vector store (same as shared_id/collection_name)",
                    example="vs_abc123")
    ...
    collection_name: str = Field(..., description="Name of the corresponding Qdrant collection (same as id)",
                                 example="vs_abc123")
    ...
    created_at: int = Field(..., description="Unix timestamp when the vector store record was created",
                            example=1640995200)
    updated_at: Optional[int] = Field(None, description="Unix timestamp when the record was last updated",
                                      example=1641081600)
    ...
    file_count: int = Field(..., ge=0, description="Number of files associated with this vector store in the DB",
                            example=10)
...
class VectorStoreFileCreate(BaseModel):
    ...
    file_path: str = Field(..., max_length=1024, description="Path identifier used in Qdrant metadata "
                                                             "" "(e.g., 'source' field).")
    # Added optional status
    status: Optional[StatusEnum] = Field(None, description="Initial status of the file (e.g., completed, queued). "
                                                           "" "Defaults to 'completed' if not provided by API.")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Optional user-defined metadata associated with the "
                                                                 "" "file.")
...
class VectorStoreFileRead(BaseModel):
    ...
    file_path: str = Field(..., description="Path identifier used in Qdrant metadata",
                           example="s3://bucket/document.pdf")
    processed_at: Optional[int] = Field(None, description="Unix timestamp when the file " 
                                                          "processing state last changed " "(completed/failed)",
                                        example=1641081600)
...
class VectorStoreSearchResult(BaseModel):
    ...
    vector_id: Optional[str] = Field(None, description="ID of the specific vector "
                                                       "" "point in Qdrant", example="uuid-...")
    store_id: Optional[str] = Field(None, description="ID of the vector store where the result "
                                                      "" "" "was found", example="vs_abc123")
...
class EnhancedVectorSearchResult(VectorStoreSearchResult):
    explanation: Optional[SearchExplanation] = Field(None, description="Details about how the score was calculated "
                                                                       "(if requested)")
...
class VectorStoreAddRequest(BaseModel):
    ...
    def check_lengths_match(self) -> "VectorStoreAddRequest":
        ...
        if not (texts_len == vectors_len == metadata_len):
            raise ValueError(f"Lengths of texts ({texts_len}), vectors ({vectors_len}), " f"and metadata "
