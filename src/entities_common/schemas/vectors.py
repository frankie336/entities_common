...
class VectorStoreFileCreate(BaseModel):
    ...
    file_path: str = Field(..., max_length=1024,
                           description="Path identifier used in Qdrant metadata (e.g., 'source' field).")
    # Added optional status
    status: Optional[StatusEnum] = Field(None,
                                         description="Initial status of the file (e.g., completed, queued). Defaults to 'completed' if not provided by API.")
    metadata: Optional[Dict[str, Any]] = Field(None,
                                               description="Optional user-defined metadata associated with the file.")
...
class VectorStoreFileRead(BaseModel):
    ...
    processed_at: Optional[int] = Field(None,
                                        description="Unix timestamp when the file processing state last changed (completed/failed)",
                                        example=1641081600)
...
class EnhancedVectorSearchResult(VectorStoreSearchResult):
    explanation: Optional[SearchExplanation] = Field(None,
                                                     description="Details about how the score was calculated (if requested)")
...
class VectorStoreAddRequest(BaseModel):
    ...
    def check_lengths_match(self) -> 'VectorStoreAddRequest':
        ...
        if not (texts_len == vectors_len == metadata_len):
            raise ValueError(
                f"Lengths of texts ({texts_len}), vectors ({vectors_len}), and metadata ({metadata_len}) must all match.")
        return self
